...
### 归一化
去除归一化后，不修改和第一次降低学习率梯度都爆炸了，再次调低学习率后，梯度没有爆炸
归一化有稳定训练的作用；没有归一化，数值变化大，容易爆炸，为了防止爆炸就必须降低学习率，由会导致学习效率低，训练慢。
### pre-norm与post-norm
从训练结果上看基本相同，AI说Pre-Norm对学习率不敏感，模型深度可以非常深 (100+ 层)，Post-Norm特征表达能力更强
